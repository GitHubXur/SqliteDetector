...
//c语言如何判断输入的字符串是否有汉字
//https://zhidao.baidu.com/question/1691741376340922268.html
int is_zh_ch(char p)
{
...
int sqltdump( char *p, uint16_t l ) {
	...
	//	else if(is_zh_ch(*p))//在编程中，c语言、java等编程语言当中，因为汉字编码不属于ascii，而是由8位的汉字编码组成，所以在编程语言当中算的是2个字符
	//	{
			//fprintf(stdout,".");
		//	fwprintf(stdout, "%c", *p);
	...
	//	}
		...
		{
			//fprintf(stdout, ".");
			fprintf(stdout, "0x%X ", *p);  //读取之后把UTF-8编码的中文字符转换为中文汉字    UTF-8编码一个汉字占用三个字节，Unicode编码一个汉字占用两个字节
		}
...
int blob_dump_to_file( struct globals *g, char *p, size_t l ) {
	...
	f = open(fn, O_WRONLY|O_CREAT|O_TRUNC );
...
int main( int argc, char **argv ) {
	...
	g->db_origin = readFileBytes(g->input_file);
